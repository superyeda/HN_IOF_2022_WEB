	<template>
		<view class="content">
			...
			<h1 v-show="id">ID:{{id}}</h1>
			<h1 v-show="list.volt1">电压1：{{list.volt1}}</h1>
			<h1 v-show="list.volt2">电压2：{{list.volt2}}</h1>
									...
									if (item.uuid.indexOf(
										...
										this.serviceId = item.uuid,
											uni.onBLECharacteristicValueChange((res) => {
												...
												if (topic === "testtopic/vlot") {this.list.volt1=data.v1;this.list.volt2=data.v2;this.id=data.id}
												if (topic === "testtopic/alert") {
													if(data.alert1)this.list.info="故障"
													if(data.alert2)this.list.info="警告"
												}
