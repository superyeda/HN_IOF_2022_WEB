<template>
	<view class="content">
		...
		<uni-card title="电量" v-show="list.eq">
			<h1>{{parseInt(list.eq)}}</h1>
		</uni-card>
		<uni-card title="电压" v-show="list.vlot1">
			<h1>{{parseInt(list.vlot1)}}</h1>
		</uni-card>
		<uni-card title="电流" v-show="list.ampere">
			<h1>{{parseInt(list.ampere)}}</h1>
		</uni-card>
		<template v-if="connectDev!==null">
			<uni-card title="开启">
				<button @click="sendBlueMsg('0')">开锁</button>
				<button @click="sendBlueMsg('1')">上锁</button>
				<button @click="sendBlueMsg('2')">鸣笛</button>
				<button @click="sendBlueMsg('3')">左转开</button>
				<button @click="sendBlueMsg('4')">右转开</button>
				<button @click="sendBlueMsg('5')">最大速度</button>
				
			</uni-card>
			<uni-card title="关闭">
				<button @click="sendBlueMsg('6')">左转关</button>
				<button @click="sendBlueMsg('7')">右转关</button>
				<button @click="sendBlueMsg('8')">停车</button>
			</uni-card>
		</template>
								...
								if (item.uuid.indexOf(
									...
									this.serviceId = item.uuid,
										uni.onBLECharacteristicValueChange((res) => {
											...
											if (topic === "testtopic/fan") this.list.fan =data.fan_spd*10
											if (topic === "testtopic/battery") {
												this.list.eq=data.eq,
												this.list.vlot1=data.vlot1
												this.ampere=data.ampere
											}
